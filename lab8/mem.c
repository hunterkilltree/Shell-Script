...
void * best_fit_allocator(unsigned int size) {
	...
	int found_smallest_fit = 0;
	size_t sort_value;
	// Sort memory regions  ( Selection Sort (O^2))
	struct mem_region * current_region = free_regions;
	struct mem_region * sort_node = NULL, *temp = NULL;

	while(current_region) {
		sort_node = current_region;
		temp = current_region->next;

		// Find smallest element
		while (temp) {
			if (temp->size <  sort_node->size) {
				sort_node = temp;
			}
		...
		if (sort_node != current_region) {
			sort_value = sort_node->size;
			sort_node->size = current_region->size;
			current_region->size = sort_value;
		}
		current_region = current_region->next;
	}
	//current_region = free_regions;
	// Debug
	...
	do {
		if (size <= current_region->size) {
			found_smallest_fit = 1;
		}else{
			current_region = current_region->next;
		}
	} while (!found_smallest_fit && current_region != NULL);


	if (found_smallest_fit) {
		struct mem_region* tmp =
